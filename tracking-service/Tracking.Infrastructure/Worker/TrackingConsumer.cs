...
namespace tracking_service.Tracking.Infrastructure.Worker
{
    public class TrackingConsumer : BackgroundService
    {
        private readonly IServiceScopeFactory _serviceScope;
        private readonly IConfiguration _iconfig;
        private readonly ILogger<TrackingConsumer> _logger;
        private IConnection _connection;
        private IChannel _channel;


        public TrackingConsumer(ILogger<TrackingConsumer> logger, IConfiguration configuration, IServiceScopeFactory serviceScopeFactory)
        {
        ...
        protected async override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                ...
                var batchLock = new BatchBlock<TrackingDTO>(1);  // gom message l?i thành 1 batch g?m 5 message khi d? thì d?y ra 1 m?ng message
                ...
                var actionLock = new ActionBlock<TrackingDTO[]>(async batch =>
                {
                    using (var scope = _serviceScope.CreateScope())
                    {
                        ...
                        // 4. Ánh x? các Event t? DTO sang Entity
                        ...
                        {
                            _logger.LogError($"L?i khi luu Batch: {ex.Message}");
                        }
                    }
                });
